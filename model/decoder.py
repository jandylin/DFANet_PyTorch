...
class Decoder(nn.Module):
    ...
    def __init__(self, n_classes=19):
        ...
        self.enc1_conv = nn.Conv2d(in_channels=48, out_channels=32, kernel_size=1)  # not sure about the out channels
        ...
        self.enc2_conv = nn.Conv2d(in_channels=48, out_channels=32, kernel_size=1)

        self.enc3_conv = nn.Conv2d(in_channels=48, out_channels=32, kernel_size=1)
        self.enc3_up = nn.UpsamplingBilinear2d(scale_factor=4)

        self.enc_conv = nn.Conv2d(in_channels=32, out_channels=n_classes, kernel_size=1)
        #self.enc_up = nn.UpsamplingBilinear2d(scale_factor=1)  # again..

        self.fca1_conv = nn.Conv2d(in_channels=192, out_channels=n_classes, kernel_size=1)
        self.fca1_up = nn.UpsamplingBilinear2d(scale_factor=4)

        self.fca2_conv = nn.Conv2d(in_channels=192, out_channels=n_classes, kernel_size=1)
        self.fca2_up = nn.UpsamplingBilinear2d(scale_factor=8)

        self.fca3_conv = nn.Conv2d(in_channels=192, out_channels=n_classes, kernel_size=1)
