...
parser.add_argument('--lr', '--learning-rate', default=2e-1, type=float,
...
def train(train_loader, model, criterion, optimizer, metric, epoch, args):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # compute output
        output = model(input)
        loss = criterion(output.view(output.shape[0], 19, -1), target.view(target.shape[0], -1), ignore_index=-1)
...
def save_checkpoint(state, is_best, filename='./checkpoints/Cityscapes.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(filename, 'Cityscapes_best.pth.tar')
